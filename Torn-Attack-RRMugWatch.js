...
(function () {
  ...
  window.addEventListener("message", function (event) {
    if (
      event.data.source &&
      event.data.source === "TornRussianRouletteHelper"
    ) {
  ...
  function attack() {
    ...
    if (url.includes("sid=attack")) {
      ...
      console.log(
        `https://api.torn.com/user/${attackId}?selections=profile,personalstats&key=${api}`
      );
      fetch(
        `https://api.torn.com/user/${attackId}?selections=profile,personalstats&key=${api}`
      )
          ...
          response
            ...
            .then(function (data) {
              ...
              if ($(joinBtn).length) {
                $(joinBtn).after(`
                            <div id='attackInfo'>
                                <br />Attacks: <font color='green'>[W] ${
                                  parseInt(data.personalstats.attackswon) || 0
                                }</font> <font color='red'>[L] ${
                  parseInt(data.personalstats.attackslost) || 0
                }</font>
                                <br />Defends: <font color='green'>[W] ${
                                  parseInt(data.personalstats.defendswon) || 0
                                }</font> <font color='red'>[L] ${
                  parseInt(data.personalstats.defendslost) || 0
                }</font>
                                <br />Drugs: ${
                                  parseInt(data.personalstats.drugsused) || 0
                                } used (${
                  parseInt(data.personalstats.xantaken) || 0
                } xan)
                                <br />Consumables: ${
                                  parseInt(
                                    data.personalstats.consumablesused
                                  ) || 0
                                } used
                                <br />Refills: ${
                                  parseInt(data.personalstats.refills) || 0
                                } used
                                <br />Networth: $${data.personalstats.networth.toLocaleString(
                                  "en"
                                )}
                                <br />Last action: ${data.last_action.relative}
                                <br />Faction: <a href='https://www.torn.com/factions.php?step=profile&ID=${
                                  data.faction.faction_id
                                }'>${data.faction.faction_name}</a>
                            </div>`);
              }
            })
            .catch((err) => {
  ...
  function getBetAmountFromElement(userStatusWrap) {
    const betAmountElement = userStatusWrap.querySelector(
  ...
  const BASE_MAX_PERCENTAGE = 0.1; // 10%
  ...
  function initBox() {
    ...
    box.innerHTML =
      '<div style="font-size: 20px; color: gold;">Phantom Scripting</div><br>';
  ...
  function refreshBox() {
    ...
    idList.forEach((data, userId) => {
      ...
      const healthStatus =
        data.healthStatus || "Error: Unable to retrieve status";
      const level = data.level !== undefined ? data.level : "N/A";
      const Age = data.age !== undefined ? data.age : "N/A";
      const betAmount =
        data.betAmount !== "N/A"
          ? data.betAmount
          : `<span style="color:red;">${data.betAmount}</span>`;
      ...
      entry.innerHTML = `
        ...
        <strong>Estimated Mug Amount:</strong> ${formatCurrency(
          minMug
        )} - ${formatCurrency(maxMug)}`;
  ...
  function monitorChanges() {
    ...
    userInfoWraps.forEach((userStatusWrap) => {
      ...
      let betAmount = "N/A";
      const betElement = userStatusWrap
        .closest(".row___CHcax")
        .querySelector(".betBlock___wz9ED");
      if (betElement) {
