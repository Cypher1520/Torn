// ==UserScript==
// @name         Event: Christmas Town Helper
...
(function () {
    ...
    respo.json().then((data) => {
      if (url.includes("christmas_town.php")) {
        ...
        if (url.includes("q=move") || url.includes("q=initMap")) {
          ...
          if (data.mapData) {
            if (
              data.mapData.inventory &&
              (settings.spawn === 1 || settings.speed === 1)
            ) {
              let obj = {};
              obj.modifier = 0;
              obj.speedModifier = 0;
              for (const ornament of data.mapData.inventory) {
                if (ornament.category == "ornaments") {
                  if (ornament.modifierType == "itemSpawn") {
                    ...
                    console.debug(
                      'CT: Unknown ornament modifier "' +
                        ornament.modifierType +
                        '"'
                    );
                  }
                }
              ...
              if (items.length > 0) {
                ...
                if (settings.count == 1) {
                  document.querySelector(".hardyNearbyChests").innerHTML =
                    '<label>Nearby Chests(0)</label><div class="content"></div>';
                  document.querySelector(".hardyNearbyItems").innerHTML =
                    '<label>Nearby Items(0)</label><div class="content"></div>';
                  settings.count = 0;
                }
              }
            }
            ...
            if (
              data.mapData &&
              data.mapData.trigger &&
              data.mapData.trigger.item
            ) {
          ...
          if (wordFixerStart) {
            ...
            if (
              data.mistakes === 6 ||
              data.message.startsWith("Congratulations")
            ) {
              hangmanStart = false;
              stopGame();
            } else {
          ...
          if (body && body.action && body.action === "start") {
            if (body.gameType) {
              ...
              if (
                ...
                isChecked("typocalypsehelper", 2)
              ) {
                if (!typeGameStart) {
                  typeGameStart = true;
                  startGame("Typocalypse Helper");
                  document
                    .querySelector(".hardyGameBoxContent")
                    .addEventListener("click", (e) => {
                      let target = e.target;
                      if (target.className === "hardyCTTypoAnswer") {
                        let input = document.querySelector(
                          "div[class^='game'] div[class^='board'] input"
                        );
                        if (input) {
        ...
        if (
          data.mapData &&
          data.mapData.cellEvent &&
          data.mapData.cellEvent.prizes
        ) {
  ...
  function addBox() {
    if (!document.querySelector(".hardyCTBox")) {
      if (
        document.querySelector(
          "#christmastownroot div[class^='appCTContainer']"
        )
      ) {
        let newBox = document.createElement("div");
        let pcHTML = `<div class="hardyCTHeader hardyCTShadow">Christmas Town Helper</div>
    ...
    if (
      pageUrl.includes("mapeditor") ||
      pageUrl.includes("parametereditor") ||
      pageUrl.includes("mymaps")
    ) {
  ...
  function checkForNPC() {
    ...
    if (npcList.length > 0) {
      for (const npc of npcList) {
        if (
          npc.querySelector("svg").getAttribute("fill").toUpperCase() ===
          "#FA5B27"
        ) {
  ...
  function ctHelperChangeHTML(array, selector) {
    ...
    if (length > 0) {
      ...
      for (const element of array) {
        newArray.push(
          `<p>${element[0]} at ${element[1]}, ${element[2]}&nbsp;</p>`
        );
      }
      if (selector == "hardyNearbyItems") {
        document.querySelector(
          "." + selector
        ).innerHTML = `<label>Nearby Items(${length})</label><div class="content">${newArray.join(
          ""
        )}</div>`;
      } else {
        document.querySelector(
          "." + selector
        ).innerHTML = `<label>Nearby Chests(${length})</label><div class="content">${newArray.join(
          ""
        )}</div>`;
      }
    } else {
      if (selector == "hardyNearbyItems") {
        document.querySelector(
          "." + selector
        ).innerHTML = `<label>Nearby Items(0)</label><div class="content"></div>`;
      } else {
        document.querySelector(
          "." + selector
        ).innerHTML = `<label>Nearby Chests(0)</label><div class="content"></div>`;
      }
    }
  }
  function applyCSS() {
    ...
    if (isChecked("accessibility_helper", 2)) {
      GM_addStyle(
        "@keyframes pulse {0% {box-shadow: 0 0 0 60px;}50% {box-shadow: 0 0 0 60px;}100% {box-shadow: 0 0 0 60px;}}"
      );
      if (isChecked("santa_helper", 2)) {
        GM_addStyle(
          `div[npcType="santa"] { border-radius: 50%; color:  #a80a0a6e; animation: pulse 2s ease-out infinite; }`
        );
      }
      if (isChecked("item_helper", 2)) {
        GM_addStyle(
          `.items-layer .ct-item img  { color: rgba(145, 135, 77, .66); border-radius: 50% ;animation: pulse 2s ease-in-out infinite; }`
        );
      }
  ...
  function wordSolver(jumbled) {
    ...
    if (wordSolution === "whereiscrimes2.0") {
      updateGame(
        '<label class="ctHelperError">Sorry! couldn\'t find the solution. ):</label>'
      );
    } else {
  ...
  function createTable() {
      ...
      document.querySelector(".hardyCTBox2").addEventListener("click", (e) => {
        if (e.target.id === "hardyctHelperSave") {
          ...
          document.querySelector(".hardyCTtextBox").innerHTML =
            '<p>Are you sure you want to delete the finds data?</p><button id="hardyCTConfirmDelete">Yes</button><button id="hardyCTNoDelete">No</button>';
        } else if (e.target.id == "hardyCTConfirmDelete") {
          let obj = { items: {} };
          localStorage.setItem("ctHelperFound", JSON.stringify(obj));
          document.querySelector(".hardyCTtextBox").innerHTML =
            '<label class="ctHelperSuccess"Data deleted!</label>';
    ...
    document.querySelector(".hardyCTBox2").innerHTML =
      '<div class="hardyCTHeader">Christmas Town Helper</div><div class="hardyCTTableBox"><div class="hardyCTbuttonBox" style="margin-top: 8px;"><input type="checkbox" class="hardyCTHelperCheckbox" id="santa_helper"  value="yes"' +
      isChecked("santa_helper", 1) +
      '><label for="santa_helper">Highlight Santa</label><br><input type="checkbox" class="hardyCTHelperCheckbox" id="npc_helper"  value="yes"' +
      isChecked("npc_helper", 1) +
      '><label for="npc_helper">Highlight other NPCs</label><br><input type="checkbox" class="hardyCTHelperCheckbox" id="typocalypsehelper" value="yes"' +
      isChecked("typocalypsehelper", 1) +
      '><label for="typocalypsehelper">Typoclypse Helper</label><br><input type="checkbox" class="hardyCTHelperCheckbox" id="item_helper"  value="yes"' +
      isChecked("item_helper", 1) +
      '><label for="item_helper">Highlight Chest and Items</label><br><input type="checkbox" class="hardyCTHelperCheckbox" id="christmas_wreath_helper"  value="yes"' +
      isChecked("christmas_wreath_helper", 1) +
      '><label for="christmas_wreath_helper">Christmas Wreath Helper</label><br><input type="checkbox" class="hardyCTHelperCheckbox" id="snowball_shooter_helper"  value="yes"' +
      isChecked("snowball_shooter_helper", 1) +
      '><label for="snowball_shooter_helper">Snowball Shooter Helper</label><br><input type="checkbox" class="hardyCTHelperCheckbox" id="santa_clawz_helper" value="yes"' +
      isChecked("santa_clawz_helper", 1) +
      '><label for="santa_clawz_helper">Santa Clawz Helper</label><br><input type="checkbox" class="hardyCTHelperCheckbox" id="word_fixer_helper" value="yes"' +
      isChecked("word_fixer_helper", 1) +
      '><label for="word_fixer_helper">Word Fixer Helper</label><br><input type="checkbox" class="hardyCTHelperCheckbox" id="hangman_helper" value="yes"' +
      isChecked("hangman_helper", 1) +
      '><label for="hangman_helper">Hangman Helper</label><br><input type="checkbox" class="hardyCTHelperCheckbox" id="accessibility_helper"  value="yes"' +
      isChecked("accessibility_helper", 1) +
      '><label for="accessibility_helper">Accessibility (Dims the highlighter and removes the blinking, for users facing discomfort due to bright color of highlighter)</label><br><input type="checkbox" class="hardyCTHelperCheckbox" id="sound_notif_helper" value="yes"' +
      isChecked("sound_notif_helper", 1) +
    ...
    if (marketValueData == "ched") {
      document.querySelector(".hardyCTTableBox").innerHTML =
        '<label class="ctHelperError">Unable to get data from the spreadsheet. Kindly refresh the page. Contact Father [2131687] if the problem persists</label>';
    } else {
      let savedData = getSaveData();
      let obj = { items: {} };
      ...
      if (savedData == obj) {
        document.querySelector(".hardyCTTableBox").innerHTML =
          '<label class="ctHelperError">You haven\'t found any items yet. Try again later!</label>';
      } else {
        let calc = {};
        calc.totalValue = 0;
        calc.count = 0;
        ...
        for (const row of array) {
          tableArray.push(
            `<tr><td><img src="/images/items/${row[0]}/medium.png", alt = "${
              row[1]
            }"></td><td>${row[1]}</td><td>${row[2]}</td><td>$${formatNumber(
              row[3]
            )}</td><td>$${formatNumber(row[4])}</td></tr>`
          );
        }
        document.querySelector(".hardyCTTable").innerHTML =
          "<table><tr><th>Image</th><th>Item Name</th><th>Amount</th><th>Price</th><th>Total</th></tr>" +
          tableArray.join("") +
          `</table><p>Total value: $${formatNumber(
            calc.totalValue
          )}</p><p> No. of Items: ${
            calc.count
          }</p><p>Average value of an item: $${formatNumber(
            Math.round(calc.totalValue / calc.count)
          )}</p>`;
      }
    }
  }
  ...
  function startGame(gameName) {
    ...
    document.querySelector(".ctHelperGameBox").innerHTML =
      '<div class="hardyCTHeader">' +
      gameName +
      ' Helper</div><div class="hardyGameBoxContent"></div>';
  }
    ...
    if (letters2 == 0) {
      ...
      for (const word of listofWords) {
        if (word.length === totalLength) {
          if (
            word.split(" ")[1] &&
            word.split(" ")[0].length == letters1 &&
            word.split(" ")[1].length == letters2
          ) {
            hangmanArray.push(word.toUpperCase());
          }
        }
  ...
  function hangmanMain() {
    ...
    let obj = {};
    let html1 =
      '<p style="font-weight: bold; font-size: 16px; margin: 8px; text-align: center;">Possible Solutions</p><p class="ctHelperSuccess">';
    for (const word of hangmanArray) {
    ...
    for (const key in obj) {
      sortable.push([
        key,
        obj[key],
        String(+((obj[key] / hangmanArray.length) * 100).toFixed(2)) +
          "% chance",
      ]);
    }
    ...
    for (var mkl = 0; mkl < limit.limit; mkl++) {
      ...
      lettersArray.push(
        `${letter[0].toUpperCase()} <label class="helcostrDoesntLikeGreenCommas">(${
          letter[2]
        })</label>`
      );
    }
    updateGame(
      html1 +
        array.join('<label class="helcostrDoesntLikeGreenCommas">, </label>') +
        '</p><p style="font-weight: bold; font-size: 16px; margin: 8px; text-align: center;">Suggested Letters</p><p class="ctHelperSuccess">' +
        lettersArray.join(
          '<label class="helcostrDoesntLikeGreenCommas">, </label>'
        ) +
        "</p>"
    );
  }
  ...
  function updateSpawnRate() {
    ...
    if (typeof spawn == "undefined" || spawn === null) {
      ...
      document.querySelector(
        ".ctHelperSpawnRate"
      ).innerHTML = `You have a spawn rate bonus of ${spawn}%.`;
    }
    if (typeof speed == "undefined" || speed === null) {
      ...
      document.querySelector(
        ".ctHelperSpeedRate"
      ).innerHTML = `You have a speed rate bonus of ${speed}%.`;
    }
  }
  ...
  function startTypo() {
    typoCD = setInterval(() => {
      let boxes = document.querySelectorAll(
        "div[class^='game'] div[class^='board'] div[class^='gift']"
      );
      ...
      if (length > 0) {
        for (const gift of boxes) {
          ...
          array.push(
            `<button class="hardyCTTypoAnswer" hardy="${phrase}">${phrase}</button>`
          );
        }
        array.reverse();
      }
