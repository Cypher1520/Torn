...
async function updateDriversList() {
  ...
  updating = true;
  $("#updating").size() < 1 &&
    $("#racingupdatesnew").prepend(
      '<div id="updating" style="color: green; font-size: 12px; line-height: 24px;">Updating drivers\' RS and skins...</div>'
    );

  const racingSkills = FETCH_RS
    ? await getRacingSkillForDrivers(driverIds)
    : {};
    ...
    if (FETCH_RS && !!racingSkills[driverId]) {
      ...
      if (!driver.querySelector(".rs-display")) {
        nameDiv.insertAdjacentHTML(
          "beforeend",
          `<span class="rs-display">RS:${skill}</span>`
        );
      }
    } else if (!FETCH_RS) {
...
function watchForDriversListContentChanges(driversList) {
  ...
  new MutationObserver(updateDriversList).observe(driversList, {
    childList: true,
  });
...
function getDriverIds(driversList) {
  return Array.from(driversList.querySelectorAll(".driver-item")).map(
    (driver) => getDriverId(driver)
  );
}
...
async function getRacingSkillForDrivers(driverIds) {
  const driverIdsToFetchSkillFor = driverIds.filter(
    (driverId) => !racingSkillCacheByDriverId.has(driverId)
  );
  for (const driverId of driverIdsToFetchSkillFor) {
    const json = await fetchRacingSkillForDrivers(driverId);
    racingSkillCacheByDriverId.set(
      +driverId,
      json && json.personalstats && json.personalstats.racingskill
        ? json.personalstats.racingskill
        : "N/A"
    );
    if (json && json.error) {
      $("#racingupdatesnew").prepend(
        `<div style="color: red; font-size: 12px; line-height: 24px;">API error: ${JSON.stringify(
          json.error
        )}</div>`
      );
      break;
...
function showSpeed() {
  if (
    !SHOW_SPEED ||
    $("#racingdetails").size() < 1 ||
    $("#racingdetails").find("#speed_mph").size() > 0
  )
    return;
  ...
  x = setInterval(function () {
    ...
    let laps = $("#racingupdatesnew")
      .find("div.title-black")
      .text()
      .split(" - ")[1]
      .split(" ")[0];
    let len = $("#racingupdatesnew")
      .find("div.track-info")
      .attr("data-length")
      .replace("mi", "");
    let compl = $("#racingdetails")
      .find("li.pd-completion")
      .text()
      .replace("%", "");

    if (last_compl >= 0) {
      let speed =
        (((compl - last_compl) / 100) * laps * len * 60 * 60 * 1000) / period;
...
function showPenalty() {
  if (
    $("#racingAdditionalContainer").find("div.msg.right-round").size() > 0 &&
    $("#racingAdditionalContainer")
      .find("div.msg.right-round")
      .text()
      .trim()
      .startsWith("You have recently left")
  ) {
    ...
    if (penalty > now) {
      ...
      $("#racingAdditionalContainer")
        .find("div.msg.right-round")
        .text("You may join an official race at " + formatTime(date) + ".");
    }
  }
}
...
function checkPenalty() {
  ...
  if (NOTIFICATIONS && penaltyLeft > 0) {
    penaltyNotif = setTimeout(function () {
      GM_notification(
        "You may join an official race now.",
        "Torn: Racing enhancements"
      );
    }, penaltyLeft);
  }
}
...
function updateSkill(level) {
  ...
  if (lastDaysRs && lastDaysRs.includes(":")) {
    ...
    dateTs.setTime(ts);
    if (
      1 * new Date(now).setUTCHours(0, 0, 0, 0) -
        1 * dateTs.setUTCHours(0, 0, 0, 0) >=
      24 * 60 * 60 * 1000
    ) {
  ...
  if (prev !== "undefined" && typeof prev !== "undefined" && level > prev) {
    ...
    if (NOTIFICATIONS)
      GM_notification(
        "Your racing skill has increased by " + inc + "!",
        "Torn: Racing enhancements"
      );
  ...
  if ($("#racingMainContainer").find("div.skill").size() > 0) {
    ...
    if (lastInc) {
      $("div.skill").append(
        `<div style="margin-top: 10px;">Last gain: ${lastInc}</div>`
      );
    }
  }
}
...
function parseRacingData(data) {
  ...
  const my_name =
    $("#sidebarroot").find("a[class^='menu-value']").html() ||
    ...
    $("#leaderBoard")
      ...
      .each(function () {
        ...
        if (name == results[i][0]) {
          const p = i + start + 1;
          const position =
            p === 1 ? "gold" : p === 2 ? "silver" : p === 3 ? "bronze" : "";
          let place;
          ...
          const result =
            typeof results[i][2] === "number"
              ? formatTimeMsec(results[i][2] * 1000)
              : results[i][2];
          const bestLap = results[i][3]
            ? formatTimeMsec(results[i][3] * 1000)
            : null;
          $(this)
            .find("li.name")
            .html(
...
function addSettingsDiv() {
  if ($("#racingupdatesnew").size() > 0 && $("#racingEnhSettings").size() < 1) {
    const div =
      '<div style="font-size: 12px; line-height: 24px; padding-left: 10px; padding-right: 10px; background: repeating-linear-gradient(90deg,#242424,#242424 2px,#2e2e2e 0,#2e2e2e 4px); border-radius: 5px;">' +
...
function addExportButton(results, crashes, my_name, race_id, time_ended) {
  if ($("#racingupdatesnew").size() > 0 && $("#downloadAsCsv").size() < 1) {
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      csv +=
        [
          ...
          timeStr,
          bestLap,
          results[i][0] === my_name ? GM_getValue("racinglevel") : "",
        ].join(",") + "\n";
    }
    ...
    const fileName = `${timeE.getUTCFullYear()}${pad(
      timeE.getUTCMonth() + 1,
      2
    )}${pad(timeE.getUTCDate(), 2)}-race_${race_id}.csv`;
...
function addPlaybackButton() {
  if (
    $("#racingupdatesnew").size() > 0 &&
    $("div.race-player-container").size() < 1
  ) {
    $("div.drivers-list > div.cont-black")
      .prepend(`<div class="race-player-container"><button id="play-pause-btn" class="play"></button>
    ...
    .each((i, el) => {
      if ($(el).text().includes("Racing")) {
        ...
        if ($(desc).size() > 0) {
          ...
          const lastDaysRs = GM_getValue("lastDaysRs");
          const oldRs =
            lastDaysRs && lastDaysRs.includes(":")
              ? lastDaysRs.split(":")[1]
              : undefined;
          $(desc).text(
            `${rsText} / Daily gain: ${
              currentRs && oldRs
                ? (1 * currentRs - 1 * oldRs).toFixed(5)
                : "N/A"
            }`
          );
          $(desc).attr(
            "title",
            "Daily gain: How much your racing skill has increased since yesterday."
          );
        }
        ...
        const oldPoints =
          lastDaysPoints && lastDaysPoints.includes(":")
            ? lastDaysPoints.split(":")[1]
            : undefined;
        let pointsTitle =
          "Racing points earned: How many points you have earned throughout your carreer.";
        for (const x of [
          { points: 25, class: "D" },
          { points: 100, class: "C" },
          { points: 250, class: "B" },
          ...
          if (currentPoints && currentPoints < x.points)
            pointsTitle += `<br>Till <b>class ${x.class}</b>: ${
              1 * x.points - 1 * currentPoints
            }`;
        }
...
${currentPoints ? currentPoints : "N/A"} / Daily gain: ${
          currentPoints && oldPoints ? 1 * currentPoints - 1 * oldPoints : "N/A"
        }
</span>
</li>`;
...
ajax((page, xhr) => {
  ...
  if (
    $("#racingAdditionalContainer").size() > 0 &&
    $("#racingAdditionalContainer").find("div.custom-events-wrap").size() > 0
  ) {
    $("#racingAdditionalContainer")
      .find("div.custom-events-wrap")
      .find("ul.events-list > li")
      .each((i, li) => {
        if (
          $(li).find("li.name").size() > 0 &&
          $(li).find("li.name").text().trim().startsWith("JLT-")
        ) {
          $(li).addClass("gold");
          $(li)
            .css("color", JltColor)
            .css("text-shadow", `0 0 1px ${JltColor}`);
...
$("#racingupdatesnew").ready(function () {
  $("div.racing-main-wrap")
    .find("ul.categories > li > a")
    .on("click", function () {
...
if (
  (FETCH_RS || SHOW_SKINS) &&
  $(location).attr("href").includes("sid=racing")
) {
  $("#racingupdatesnew").ready(function () {
    updateDriversList();
    // On change race tab, (re-)insert the racing skills if applicable:
    new MutationObserver(updateDriversList).observe(
