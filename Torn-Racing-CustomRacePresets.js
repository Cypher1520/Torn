// ==UserScript==
// @name         Racing: Custom Race Presets
...
function PresetHandler() {
  ...
  function InjectSaveButton() {
    ...
    $saveButton.on("click", function () {
      ...
      presets.push(race);
      const btn = $(
        `<button class="torn-btn preset-btn">${race.name}</button>`
      );
  ...
  function fillPreset(race) {
    if (!!race.name)
      $(".race-wrap div.input-wrap input").attr("value", race.name);
    if (!!race.minDrivers)
      $(".drivers-min-wrap div.input-wrap input").attr(
        "value",
        race.minDrivers
      );
    if (!!race.maxDrivers)
      $(".drivers-max-wrap div.input-wrap input").attr(
        "value",
        race.maxDrivers
      );
    if (!!race.numberOfLaps)
      $(".laps-wrap > .input-wrap > input").attr("value", race.numberOfLaps);
    if (!!race.betAmount)
      $(".bet-wrap > .input-wrap > input").attr("value", race.betAmount);
    if (!!race.waitTime)
      $(".time-wrap > .input-wrap > input").attr("value", race.waitTime);
    if (!!race.password)
      $(".password-wrap > .input-wrap > input").attr("value", race.password);
    if (!!race.trackName) {
      $("#select-racing-track").selectmenu();
      $(
        "#select-racing-track-menu > li:contains(" + race.trackName + ")"
      ).mouseup();
    }
    if (!!race.upgradesAllowedString) {
      $("#select-allow-upgrades").selectmenu();
      $(
        "#select-allow-upgrades-menu > li:contains(" +
          race.upgradesAllowedString +
          ")"
      ).mouseup();
    }
  }
  ...
  function normalizePreset(item, index = 0) {
    ...
    if (!!item.maxDrivers)
      item.maxDrivers =
        item.maxDrivers > 100 ? 100 : item.maxDrivers < 2 ? 2 : item.maxDrivers;
    if (!!item.minDrivers)
      item.minDrivers =
        item.minDrivers < 2
          ? 2
          : item.minDrivers >= item.maxDrivers
          ? item.maxDrivers - 1
          : item.minDrivers;
    if (!!item.trackName)
      item.trackName = item.trackName
        ...
        .join(" ");
    if (!!item.numberOfLaps)
      item.numberOfLaps =
        item.numberOfLaps > 100
          ? 100
          : item.numberOfLaps < 1
          ? 1
          : item.numberOfLaps;
    item.upgradesAllowedString =
      !item.hasOwnProperty("upgradesAllowed") || !!item.upgradesAllowed
        ? "Allow upgrades"
        : "Stock cars only";
    if (!!item.betAmount)
      item.betAmount =
        item.betAmount > 10000000
          ? 10000000
          : item.betAmount < 0
          ? 0
          : item.betAmount;
    if (!!item.waitTime)
      item.waitTime =
        item.waitTime > 2880 ? 2880 : item.waitTime < 1 ? 1 : item.waitTime;
    if (!!item.password && item.password.length > 25)
      item.password = item.password.substring(0, 26);
    return item;
  }
  ...
  function getCustomRaceForm() {
    return $("#createCustomRace")
      .serializeArray()
      .reduce(function (race, cur) {
        switch (cur.name) {
          case "title":
            race.name = cur.value;
            break;
          case "maxDrivers":
            race.maxDrivers = parseInt(cur.value);
            break;
          case "minDrivers":
            race.minDrivers = parseInt(cur.value);
            break;
          case "trackID":
            race.trackName = TRACKS[cur.value];
            break;
          case "laps":
            race.numberOfLaps = parseInt(cur.value);
            break;
          case "betAmount":
            race.betAmount = parseInt(cur.value);
            break;
          case "waitTime":
            race.waitTime = parseInt(cur.value);
            break;
          case "password":
            race.password = cur.value;
            break;
          case "carsTypeAllowed":
            race.upgradesAllowed = cur.value === "1";
            break;
        }
        return race;
      }, {});
