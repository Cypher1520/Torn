// ==UserScript==
// @name        War: Faction Wall Battlestats +color by zim312
// @description Show tornstats spies on faction wall page by finally, color by zim312
// @namespace   finally+zim312.torn.FactionWallBattlestats
// @match       https://www.torn.com/factions.php*
// @run-at      document-end
// @grant       GM_addStyle
// @grant       GM_xmlhttpRequest
// @downloadURL  https://zntc.tk/scripts/factionwar.user.js
// @updateURL    https://zntc.tk/scripts/factionwar.user.js
// @version     3.25
// @author      finally & zim312
// @connect 	tornstats.com
// @connect 	api.torn.com
// ==/UserScript==

let apiKey = localStorage["finally.torn.api"] || null;
//Edit this value to your own stats
let myBS = localStorage["finally.torn.battlestats"] || 2000000000;
let myBStime = localStorage["finally.torn.battlestattime"] || 0;
let bsCache = JSONparse(localStorage["finally.torn.bs"]) || {};
let hospTime = {};
let previousSort =
  parseInt(localStorage.getItem("finally.torn.factionSort")) || undefined;
let filterFrom =
...
async function updateBS(key) {
  let url = `https://api.torn.com/user/?selections=battlestats&key=${key}&comment=FactionWar`;
  let tmpdata = await fetch(url);
  let itemdata = await tmpdata.json();
  console.log(itemdata);
  let bstats = itemdata.total.toString();
  let myDate = Date.now();
  localStorage.setItem("finally.torn.battlestats", bstats);
  localStorage.setItem("finally.torn.battlestattime", myDate);
}
  ...
  GM_xmlhttpRequest({
    ...
    onload: (r) => {
      ...
      if (!j.status) {
        ...
        if (myBStime < Date.now() - 2629800000) {
          updateBS(key);
        }
        cb(true);
      }
...
function loadFactions() {
  let factionIds = Array.from(
    document.querySelectorAll("[href^='/factions.php?step=profile']")
  )
...
function updateStats(id, node, parentNode) {
  ...
  let dark = false;
  dark = document.querySelector("body").classList.contains("dark-mode");
  ...
  if (bsCache[id] && bsCache[id].total > myBS * 0.75) {
    node.innerHTML =
      `<span style="color: ${yellow}">` + stats[0] + `&nbsp;</span>`;
  }
  if (bsCache[id] && bsCache[id].total < myBS * 0.75) {
    node.innerHTML =
      `<span style="color: ${green}">` + stats[0] + `&nbsp;</span>`;
  }
  if (bsCache[id] && bsCache[id].total > myBS * 1.25) {
    node.innerHTML =
      `<span style="color: ${red}">` + stats[0] + `&nbsp;</span>`;
  }
  node.title = `
...
function showStats(node) {
  ...
  let isMobile = false;
  bsNode.addEventListener("touchstart", () => {
    isMobile = true;
  });
...
function watchWall(observeNode) {
  ...
  const mo = new MutationObserver((mutations) => {
    ...
    if (
      ...
      previousSort = Array.from(observeNode.querySelectorAll('a[href*="XID"]'))
        .map((a) => a.href)
        .join(",");
      mo.takeRecords();
...
new MutationObserver((mutations) => {
  mutations.forEach((mutation) => {
    for (const node of mutation.addedNodes) {
      ...
      addAPIKeyInput(
        node.querySelector && node.querySelector(".content-title")
      );
    }
  });
