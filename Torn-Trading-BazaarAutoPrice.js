      ...
      if (typeof node.classList !== "undefined" && node.classList) {
        ...
        if ($(input).size() > 0 && $(remove).size() > 0) {
          ...
          if (input && img) {
            ...
            // input amount
            const input_amount = $(node)
              .find("div.amount")
              .find(".clear-all[type=text]");
            const inv_amount = $(node)
              .find("div.name-wrap")
              .find("span.t-hide")
              .text();
            const amount =
              inv_amount == "" ? 1 : inv_amount.replace("x", "").trim();
