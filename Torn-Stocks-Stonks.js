...
(function () {
    ...
    if (socket.url === "wss://ws-centrifugo.torn.com/connection/websocket") {
      socket.addEventListener("message", (t) => {
        ...
        if (
          data.result &&
          data.result.channel &&
          data.result.channel == "stockMarket"
        ) {
          ...
          if (msg.action == "refreshStocksPrice") {
            ...
            if (pageurl.includes("link=hardy")) {
              document.querySelector("#hardyPortfolioBox").innerHTML =
                returnHtml();
            } else {
              addProfitLossInfo();
              changeTitle();
              document.querySelector(".hardy_stonks_text_info").innerHTML = `${
                stonksTotalVal >= 0
                  ? '<span class="stonksUpli">Current Profit: '
                  : '<span class ="stonksDownli">Current Loss: '
              }${formatNumber(
                stonksTotalVal
              )}</span><br>Current Holdings: ${formatNumber(
                moneyInvested
              )}<br>Invested Into ${investedIn} Stocks`;
            }
          ...
          if (
            msg.namespaces &&
            msg.namespaces.sidebar &&
            msg.namespaces.sidebar.actions &&
            msg.namespaces.sidebar.actions.updateMoney
          ) {
    ...
    if (url.includes("page.php?sid=StockMarket&step=getInitialData")) {
      ...
      respo.json().then((info) => {
        if (info.success) {
          ...
          let transList = document.querySelectorAll(
            "div[class^='transactionsList'] ul[class^='transaction_']"
          );
          if (transList.length > 0) {
            ...
            for (const trans of transList) {
              let amount = parseInt(
                trans
                  .querySelector("li[class^='shares']")
                  .innerText.replace(/,/g, "")
              );
              let buyP = parseFloat(
                trans
                  .querySelector("li[class^='bought']")
                  .innerText.replace(/\$/, "")
                  .replace(/,/g, "")
              );
              ...
              portfolioData[id].push([
                id,
                dateToStamp(trans.querySelector("li[class^='date']").innerText),
                obj.stock,
                amount,
                buyP,
                buyTotal,
      ...
      respo.json().then((info) => {
        if (info.success) {
          ...
          for (const id in metadata) {
            if (
              metadata[id].days === metadata[id].progress &&
              metadata[id].type === "active"
            ) {
          ...
          setTimeout(() => {
            ...
            if (transList.length > 0) {
              ...
              for (const trans of transList) {
                ...
                let total = Math.round(price * amount);
                portfolioData[uid].push([
                  uid,
                  dateToStamp(
                    trans.querySelector("li[class^='date']").innerText
                  ),
                  metadata[uid].acronym,
                  amount,
                  buyP,
                  buyTotal,
                  price,
  ...
  function addIndex() {
    ...
    for (const id in metadata) {
      let node = document.querySelector(
        `li[aria-label*="${metadata[id].name}"]`
      ).parentNode;
    ...
    if (!pageurl.includes("link=hardy")) {
      ...
      icon.setAttribute(
        "href",
        "https://www.torn.com/page.php?sid=stocks&link=hardyportfolio"
      );
      ...
      div.innerHTML = `<div class="hardy_stonks_box_header">Stonks</div><div class="hardy_stonks_box"><div class="stonksBulletin"><label class="hardy_stonks_text_info">${
        stonksTotalVal >= 0
          ? '<span class="stonksUpli">Current Profit: '
          : '<span class ="stonksDownli">Current Loss: '
      }${formatNumber(
      ...
      for (const acronym of acrArray) {
        array.push(
          `<option value="${acronym.toLowerCase()}">${acronym}</option>`
        );
      }
  ...
  function returnHtml() {
    let html1 =
      '<table id="hardyPortfolioTable"><thead><tr><th></th><th>Name</th><th>Info</th></tr></thead>';
    ...
    for (let blocks in portfolioData) {
      for (const block of portfolioData[blocks]) {
        ...
        array.push(
          `<tr><td class="stonkLogo"><img src="${img}"></td><td class="stonkAcr">${
            block[2]
          }</td><td class="stonkInfo"><table id="innerTable" style="border-style: hidden;"><tr><td style="border-style:hidden;"><ul class="hardyStonksList1"><li class="stonkAmount"><span class="bold">Shares: </span>${formatNum(
            block[3]
          )}</li><li class="stonkzCurrentPerShare" stockId="a${
            block[0]
          }"><span class="bold">Current: </span>${formatNumber(
            block[6]
          )}</li><li><span class="bold">Length: </span>${
            block[1]
          }</li></ul></td><td style="border-style:hidden;"><ul class="hardyStonksList2"><li class="stonkzWorth"><span class="bold">Worth: </span>${formatNumber(
            block[7]
          )}</li><li><span class="bold">Bought: </span>${formatNumber(
            block[4]
          )}</li><li class="${change}li"><span class="bold">Change: </span><span class="${change}"></span><span>${formatNumber(
            block[8]
          )} (${+((block[7] / block[5]) * 100 - 100).toFixed(
            2
          )}%)</span></li></ul></td></tr></table></td></tr>`
        );
      }
    }
    ...
    for (const node of nodeList) {
      ...
      if (savedPrefs[id]) {
        ...
        if (
          ...
          savedPrefs[id].higherThan < price
        ) {
          node.setAttribute("isGreen", "yes");
      ...
      if (length1 > 1) {
        if (length1 > 2) {
          ...
          document.querySelector(
            "#lowerBulletin"
          ).innerText = `Stocks ${lowerArray.join(
            ", "
          )} and ${lastElem} are at prices lower than your selected values.`;
        } else {
          document.querySelector(
            "#lowerBulletin"
          ).innerText = `Stocks ${lowerArray[0]} and ${lowerArray[1]} are at prices lower than your selected values.`;
        }
      } else {
        document.querySelector(
          "#lowerBulletin"
        ).innerText = `Stock ${lowerArray[0]} is at a price lower than your selected value.`;
      }
    } else {
    ...
    if (length2 > 0) {
      if (length2 > 1) {
        if (length2 > 2) {
          ...
          document.querySelector(
            "#higherBulletin"
          ).innerText = `Stocks ${higherArray.join(
            ", "
          )} and ${lastElem} are at prices higher than your selected values.`;
        } else {
          document.querySelector(
            "#higherBulletin"
          ).innerText = `Stocks ${higherArray[0]} and ${higherArray[1]} are at prices higher than your selected values.`;
        }
      } else {
        document.querySelector(
          "#higherBulletin"
        ).innerText = `Stock ${higherArray[0]} is at a price higher than your selected value.`;
      }
    } else {
    ...
    if (length > 0) {
      if (length > 1) {
        if (length > 2) {
          ...
          document.querySelector(
            "#readyPayout"
          ).innerText = `Stocks ${array.join(
            ", "
          )} and ${lastElem} are ready for payout.`;
        } else {
          document.querySelector(
            "#readyPayout"
          ).innerText = `Stocks ${array[0]} and ${array[1]} are ready for payout.`;
        }
      } else {
        document.querySelector(
          "#readyPayout"
        ).innerText = `Stock ${array[0]} is ready for payout.`;
      }
    } else {
  ...
  function handleSelectInput(value) {
    ...
    if (value === "owned") {
      ...
      if (savedPrefs.prefs.irr_list) {
        ...
        document.querySelector(
          "#selectOutput"
        ).innerHTML = `<label class="stonksDownli">Please create a list of irrelevant stocks first by clicking on Settings icon.</label>`;
      }
    }
  }
  ...
  function createAmountInput() {
    ...
    if (mainDiv) {
      ...
      let manageBlock = mainDiv.querySelector(
        "div[class^='manageTab_'] div[class^='manageContainer']"
      );
      if (manageBlock && !manageBlock.querySelector(".buyHint")) {
        let buyDiv = manageBlock.querySelector(
          "div[class^='buyBlock'] div[class^='manageBlock']"
        );
        if (buyDiv.querySelector(".input-money-group")) {
          ...
          let div = document.createElement("div");
          div.innerHTML = `<p class="buyHint">Enter amount of money you want to spend:</p><div style="display:inline;"><label class="hardy_amount_max">$</label><input type="text" class="hardy_stonk_buy_input" info="${id}" value="${formatNum(
            moneyOnHand
          )}"><label class="money_to_quant">FILL</label></div>`;
  ...
  function sendManual(str) {
    ...
    let stamp =
      dayStamp +
      parseInt(timeHMS[2]) +
      parseInt(timeHMS[1]) * 60 +
      parseInt(timeHMS[0]) * 3600;
    let obj = {};
    obj.key = "hardy";
    ...
    document.querySelector(
      "#selectOutput"
    ).innerHTML = `<label class="stonksUpli">Data sent to your webapp.</label>`;
  }
  ...
  function addListeners() {
    ...
    document
      .querySelector(".hardy_stonks_box")
    ...
    document.querySelector("#selectOutput").addEventListener("click", (g) => {
      ...
      if (target.id === "sendManual2") {
        ...
        let str = val.trim();
        let buyRegex =
          /[0-9]{2}:[0-9]{2}:[0-9]{2} - [0-9]{2}\/[0-9]{2}\/[0-9]{2} You bought ([\d,]+)x (.+) (?:shares|share) at \$([\d,.]+) each for a total of \$([\d,]+)/gm;
        let sellRegex =
          /[0-9]{2}:[0-9]{2}:[0-9]{2} - [0-9]{2}\/[0-9]{2}\/[0-9]{2} You sold ([\d,]+)x (.+) (?:shares|share) at \$([\d,.]+) each for a total of \$([\d,]+) after \$([\d,]+) in fees/gm;
        if (buyRegex.test(str) || sellRegex.test(str)) {
          sendManual(str);
        } else {
          document.querySelector(
            "#selectOutput"
          ).innerHTML = `<label class="stonksDownli">Please copy the transaction log properly from activity log and make sure time is also included.</label>`;
        }
    ...
    document
      .querySelector("div[class^='stockMarket_']")
      .addEventListener("input", function (g) {
    ...
    document
      ...
      .addEventListener("click", (t) => {
        ...
        if (target.className === "hardy_amount_max") {
          document.querySelector(".hardy_stonk_buy_input").value =
            formatNum(moneyOnHand);
          ...
          if (inputBox.getAttribute("isError") !== "yes") {
            ...
            let outputBox = document.querySelector(
              "div[class^='buyBlock'] .input-money"
            );
      ...
      if (!target.id) {
        ...
        if (target.id === "saveCondi") {
          ...
          if (buyRegex.test(str) || sellRegex.test(str)) {
            document.querySelector(
              "#selectOutput"
            ).innerHTML = `<label>Are you sure you want to log that transaction into the spreadsheet?</label><br><br><button id="sendManual2">Yes</button><button id="cancelManualSend">No</button>`;
          } else {
            document.querySelector(
              "#selectOutput"
            ).innerHTML = `<label class="stonksDownli">Please copy the transaction log properly from activity log and make sure time is also included.</label>`;
          ...
          let nodeList = document.querySelectorAll(
            ".hardy_stonks_hideList:not(.hardyStonkHidden)"
          );
          for (const node of nodeList) {
          ...
          let nodeList = document.querySelectorAll(
            ".hardy_stonks_hideList.hardyStonkHidden"
          );
          for (const node of nodeList) {
          ...
          document.querySelector(
            "#selectOutput"
          ).innerHTML = `<label class="stonksUpli">Data saved!!</label>`;
        }
      }
    });
    document.querySelector("#prefBox_stonks").addEventListener("click", (g) => {
      ...
      if (target.id === "openprompt") {
        document.querySelector(
          "#selectInput"
        ).innerHTML = `<label>Enter Webapp link: </label><input type="text" id="stonksLink" value="${savedPrefs.link}"><button id="saveLink">Save</button><button id="closeLink">Close</button>`;
        document.querySelector("#selectOutput").innerHTML = ``;
        ...
        for (const acr of acrArray) {
          if (savedPrefs.prefs.irr_list.indexOf(acr) !== -1) {
            array.push(
              `<span class="hardy_stonks_hideList hardyStonkHidden">${acr}</span>`
            );
          } else {
        ...
        document.querySelector(
          "#selectInput"
        ).innerHTML = `<label style="font-size: 15px; font-weight:bold; display: block;">Click on the Stocks you want to hide from list:</label>${array.join(
          ""
        )}<br><br><button id="selectAllStonks">Select All</button><button id="unselectAllStonks">Unselect All</button><button id="saveHiddenStonks">Save</button><button id="closeLink">Close</button>`;
        document.querySelector("#selectOutput").innerHTML = ``;
      }
    });
    document.querySelector(".hardy_icon").addEventListener("click", () => {
      ...
      document.querySelector(
        "#prefBox_stonks"
      ).innerHTML = `<button id="openprompt">Webapp</button><button id="stonks_hide_list">Irrelevant List</button><button id="stonkPrefClose"> Close</button>`;
    });
    ...
    document.querySelector("#manualEntry").addEventListener("click", () => {
      ...
      document.querySelector(
        "#selectInput"
      ).innerHTML = `<input id="stonks_manual_input" type="text"><button id="sendManual1">Send</button><button id="closeLink">Close</button>`;
    });
    document
      .querySelector(".hardy_stonks_box_header")
